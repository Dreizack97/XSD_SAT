...
namespace CFDI_Reader.Utilities
{
    ...
    {
        public static void GeneratePdf33(string path, XSD.Comprobante.V33.Comprobante comprobante)
        {
            ...
            tableBody.AddCell(new Cell().Add(new Paragraph("")).SetBorder(NO_BORDER).SetPadding(0));
            tableBody.AddCell(new Cell().Add(new Paragraph("Régimen ")).SetBorder(NO_BORDER).SetFont(normal).SetFontSize(8).SetPadding(0));
            ...
            tableBody.AddCell(new Cell().Add(new Paragraph("")).SetBorder(NO_BORDER).SetPadding(0));
            tableBody.AddCell(new Cell().Add(new Paragraph("Domicilio" + " Entidad Federativa " + comprobante.Nomina.Receptor.ClaveEntFed)).SetBorder(NO_BORDER).SetFont(normal).SetFontSize(8).SetPadding(0));
            ...
            string url = "https://verificacfdi.facturaelectronica.sat.gob.mx/default.aspx?id=" + comprobante.TimbreFiscalDigital.UUID + "&re=" + comprobante.Emisor.Rfc + "&rr=" + comprobante.Receptor.Rfc + "&tt=" + comprobante.Total + "&fe=" + comprobante.TimbreFiscalDigital.SelloCFD?.Substring(336);

            Image QRImage = GenerateQrImage(url);
            ...
            tablaFiscal.AddCell(new Cell(1, 7).Add(new Paragraph("Este documento es una representación impresa de un CFDI versión 4.3")).SetBorder(NO_BORDER).SetFont(normal).SetFontSize(6).SetTextAlignment(CENTER));
        ...
        public static void GeneratePdf40(string path, XSD.Comprobante.V40.Comprobante comprobante)
        {
            PdfDocument pdf = new PdfDocument(new PdfWriter(path.ToUpper().Replace(".XML", ".pdf")));
            Document document = new Document(pdf);
